...
@Injectable()
export class ___Month_OLD_Store {
	...
	public dispatchYearMonth(year: number, month: number): void {
		this.state = MonthReducers.reduceYearMonth(
			this.state,
			year,
			month
		);
	...
	public dispatchGetMonthBalances(): void {
		this.controlApi.getMonthBalancesList$().subscribe(res => {
			this.state = MonthReducers.reduceGetMonthBalances(
				this.state,
				res
			);
	...
	public dispatchSetGoalMonth(goal: number): void {
		this.state = MonthReducers.reduceSetGoalMonth(
			this.state,
			goal
		);
	...
	public dispatchGetJournalEntries(): void {
		this.controlApi.getJournalEntriesList$().subscribe(res => {
			this.state = MonthReducers.reduceJournalEntries(
				this.state,
				res
			);
			...
			this.expenses$.next(
				MonthReducers.filterJournalsByKind(this.state, "E")
			);
		});
	}
	public dispatchPostJournalEntry(
		journalEntry: JournalEntry
	): void {
		this.controlApi
			.postJournalEntry$(journalEntry)
			.subscribe(res => {
				this.state = MonthReducers.reducePostJournalEntry(
					this.state,
					res
				);
	...
	public dispatchDeleteJournalEntry(
		journalEntry: JournalEntry
	): void {
		this.controlApi
	...
	private dispatchPostMonthBalance(): void {
		this.controlApi
			...
			.subscribe(res => {
				this.state = MonthReducers.reducePostMonthBalance(
					this.state,
					res
				);
				this.monthBalance$.next(this.state.monthBalance);
	...
	private dispatchPutMonthBalance(): void {
		this.state = MonthReducers.calculateMonthBalance(
			this.state
		);
		this.controlApi
			.putMonthBalance$(this.state.monthBalance)
			.subscribe(res => {
				this.state = MonthReducers.reducePutMonthBalance(
					this.state,
					res
				);
				this.monthBalance$.next(this.state.monthBalance);
